  ...
  return (
    <Suspense fallback={null}>
      <>
        <span
          draggable={draggable}
          className={cn(position === "left" && "mr-2", position === "right" && "ml-2")}
        >
          <div className={cn("relative mx-auto", position === "full" && "w-max")}>
            {isEditable && (
              <Button
                className="image-edit-button !text-background/90 hover:!text-background absolute top-1 right-1"
                variant="outline"
                ref={buttonRef}
                onClick={() => {
